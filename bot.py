    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get('status') == 'ok':
                video_url = result.get('links', {}).get('nowatermark')
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not url:
        await query.message.edit_text("❌ الرابط غير موجود، أرسل الرابط مرة أخرى")
        return
    ...
    try:
        ...
        if download_type == 'video':
            ...
            if file_path and os.path.exists(file_path):
                ...
                with open(file_path, 'rb') as f:
                    await message.reply_audio(audio=f, title=info['title'], performer="TikTok - برعاية أياد")
                os.remove(file_path)
            else:
                stats['failed'] += 1
                save_stats(stats)
...
def main():
    print("🚀 تشغيل بوت تحميل تيك توك على Railway...")
